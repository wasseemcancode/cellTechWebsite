...
} from '/var/task/cellTechWebsite/celltechuf/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer } from '/var/task/cellTechWebsite/celltechuf/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/cellTechWebsite/celltechuf')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return tracer.withActiveSpan('Next.js Server Handler', async (span) => {
      span.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/cellTechWebsite/celltechuf',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/cellTechWebsite/celltechuf/.netlify/dist/run/handlers/server.js')
